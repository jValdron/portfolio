  ...
  module.exports = function(callback){
    ...
    async.series([
      ...
      function(next){
        ...
        if (fs.existsSync(path.join(process.cwd(), 'config', process.env.NODE_ENV))) {
          ...
          if (process.env.DATABASE_URL) {
            ...
            var info = process.env.DATABASE_URL.match(/([A-z].+):\/\/([A-z0-9\.].+):([A-z0-9\.].+)@([A-z0-9\.].+):([0-9].+)\/(.*)/);
      ...
      async.parallel([
        ...
        // HTTP to HTTPS endpoint for production which supports HTTPS.
        function(done){

          if (process.env.NODE_ENV === 'production') {

            app.logger.info('Configuring HTTP to HTTPS route...');

            // Force HTTPS requests.
            app.get('*', function(req, res, next){
              if (req.headers['x-forwarded-proto'] === 'https' || req.url === '/ping') {
                return next();
              } else {

                // HTTPS redirect is permanent.
